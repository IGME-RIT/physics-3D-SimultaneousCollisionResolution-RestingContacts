...
// This is similar to the other pipelines,
// except with different shaders, and a different
// vertex type

#include "VertexBasic.h"

Pipe2D::Pipe2D(VkDevice dev, VkDescriptorSetLayout desc_layout, VkRenderPass render_pass)
{
	device = dev;
	...
	// This will be empty, because we are not using vertex buffers
	...
	// we set Topology to TRIANGLE_STRIP, so that we can draw 
	// one quad with four vertices (two triangles) rather than 6
